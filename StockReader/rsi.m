...
%% Indicators
ri = rsindex(price, N);
SMA = tsmovavg(price','s',150);

d_SMA = gradient(SMA); % Gradient of SMA
...
for i = 2:length(s)
%      if d_SMA(i) > 0;
        if  ri(i) < thresh(2) && ri(i-1) > thresh(2)
            bool = false;
        elseif ri(i) < thresh(1) && ri(i-1) > thresh(1)
            bool = false;
        elseif ri(i) > thresh(1) && ri(i-1) < thresh(1) || bool
            s(i) = 1;
            bool = true;
        end
%      end
...
if nargout == 0
    ...
    for i = 1:nargout
        switch i
            ...
            case 3
                varargout{3} = ri; % rsi signal
            case 4
                varargout{4} = SMA; % Single Moving Average
            case 5
                varargout{5} = sh; % moving average
            case 6
